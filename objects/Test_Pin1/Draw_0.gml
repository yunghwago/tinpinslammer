...
//debug - drawing vector arrows
if (global.debugmode){
	draw_set_colour(c_blue);
	draw_arrow(x,y,x+vx*10,y+vy*10,30);
	draw_set_color(c_red);
	//draw_arrow(x,y,x+vx*10*dcos(collisionangle),y+vy*10* -dsin(collisionangle),30);
	draw_set_colour(c_white);
	draw_text(x+25,y-25,"VX = " + string(vx) + "\n VY = " + string(vy));
	draw_set_colour(c_lime);
	draw_line_width(x,y,x+dcos(theta)*25,y-dsin(theta)*25,2);
	draw_set_colour(c_black);
	draw_line(Test_Pin1.x,Test_Pin1.y,object1.x,object1.y);
}
